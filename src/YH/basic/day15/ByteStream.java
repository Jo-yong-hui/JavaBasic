...
public class ByteStream {
    public static void main(String[] args) {
        ...
        // 파일을 바이트 단위로 입출력하려면
        // InputStream 또는 OutputStream 클래스

        // 바이트기반으로 스트림을 읽거나 씀: 바이트 기반(1byte)
        // 주로 바이너리(이진) 기반 데이터(이미지, 동영상등)을 처리할때 사용
        ...
        try {
            ...
            // fos 1빠 bos 2빠 osw3빠 입력
            // 닫는(close) 순서는 LIFO (마지막 입력된것부터 순서데로 close한다)
        ...
        try{
            ...
            isr = new InputStreamReader(bis); //한글처리하는 isr 덕분에 파일읽기2 결과처럼 한글이 안꺠짐
            int ch = 0;
            String result = "";
            ...
            // InputStreamReader 덕분에 한글은 안깨지고 제대로 출력
        }catch (Exception ex){
            ex.printStackTrace();
        }
        ...
        try{
            ...
            System.out.println(result);
            // 여기선 한글처리 기능(InputStreamReader)이 없으므로 한글이 깨짐
