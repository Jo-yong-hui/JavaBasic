...
public class Exceptions {
    public static void main(String[] args) throws IOException {
        ...
        // 일반예외 : checek exception
        // 명시적인 예외처리를 반드시 강제함 (!!)
        // 예외발생 시점은 컴파일할떄 compile time임
        // IOException
        // ClassNotFoundException
        
        // ex) 임의의 성적 데이터(이름, 국어, 영어, 수학)을 입력받아
        // c:/Java/sungjuk.txt에 저장하는 프로그램 작성
        String sjdata = "혜교 98 45 23";
        String fpath = "c:/Java/sungjuk.txt";

       //FileWriter fw = null; //이거쓴거를 생략하고 try밑에 fw 앞에 FileWriter입력
        //file기억하려고할떄 사용하는 초기 코드 작업 //담에 자세히 배움
        try {
            //데이터 입출력 코드
            FileWriter fw = new FileWriter(fpath); //"c:/Java/sungjuk.txt";파일 기록 준비 //문자단위기록 속도느림
            BufferedWriter bw = new  BufferedWriter(fw); //행단위 기록으로 속도가 빠름
            bw.write(sjdata); //"c:/Java/sungjuk.txt";에다가 성적데이터를 써라
            bw.close();
            fw.close();
        } catch (IOException e) {
            e.printStackTrace();
        ...
        bw.close();
        fw.close();
        // FileWriter 마우스데면 위에 쓴거랑 복붙한거라 빨간줄뜸. 마우스대고 더보기 누른후 예외를 메서드 시그니처에 추가
        //public static void main(String[] args) throws IOException { 만들어짐
