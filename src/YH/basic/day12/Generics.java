...
public class Generics {
    public static void main(String[] args) {
        ...
        // 컬렉션 클래스에 컴파일시 자료형 검사를 해주는 기능
        // 즉, 클래스에 사용할 타입의 정보를 동적으로 넘겨줄수 있고
        // 실행할때(runtime) 발생할 수 있는 타입간 오류를 컴파일할때에
        // 발견할 수 있도록 해주는 기능
        // JDK 1.5에서 부터 추가된 기능
        ...
        // 야생동물 3마리를 동물원에 가둡시다!
         ...
         // Object형으로 배열을 선언하면
        // 어떤 데이터 타입의 객체든지 간에 모두 저장 가능
         Object[] zoo1 = new Object[3];
         zoo1[0] = Lion1;
         zoo1[1] = tiger1;
         zoo1[2] = Zebra1;

         //단, 배열에서 객체를 빼낼떄는 그 데이터타입에 맞는
        // 적절한 형변환이 필요함
        ( (Lion) zoo1[0]).sayName();
       ...
       // ( (Tiger) zoo1[2]).sayName();
        ...
        // 컴파일할떄는 아무 오류 없다가
        // 실행할떄서야 비로소 오류가 출력됨
        // "456"문자열을 숫자로 변환할수 없어서 오류

        // JDK1.5에서 도입한 제네릭을 활용하면
        // 컴파일할때 타입이 정해지므로 
        // 타입검사나 타입변환과 같은 번거로움이 사라짐
        // (다이아몬드기호)<>기호 안에 타입변수를 지정해서 사용
        ...
        //num2라는 배열변수에는 String타입 값만 저장할 수 있음 // 검사시 Strigng 문자열 형태만 사용가능한걸 검사
        //num2객체는 String타입만 가능하다.
